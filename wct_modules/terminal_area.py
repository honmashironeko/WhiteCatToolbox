...
class TerminalArea(QWidget):
    ...
    def build_tool_command(self, tool_info, parameters):
        """构建工具执行命令"""
        from .utils import get_system_python_executable, get_configured_python_executable
        ...
        if executable.endswith('.exe') or executable == script_path:
            ...
            if executable == 'python' or executable == 'python3':
                configured_python = get_configured_python_executable(tool_info)
                if configured_python:
                    command_parts = [configured_python, script_path]
                else:
